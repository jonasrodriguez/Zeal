...
void AutoAbility::SetEnabled(int ability_slot, bool do_print) {
  if (ability_slot < 1 || ability_slot > 10) {
    if (do_print) Zeal::Game::print_chat(USERCOLOR_SPELL_FAILURE, "AutoAbility: slot must be between 1 and 10.");
    return;
  }
  ...
  if (do_print) Zeal::Game::print_chat(USERCOLOR_ECHO_SHOUT, "AutoAbility enabled for slot %d.", ability_slot);
}

void AutoAbility::SetDisabled(bool do_print) {
  if (do_print && is_active) Zeal::Game::print_chat(USERCOLOR_ECHO_SHOUT, "AutoAbility disabled.");
